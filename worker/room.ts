...
export class RoomDurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (request.method === 'POST' && parts.at(-1) === 'initialize') {
        ...
        await this.save(s); console.log('room_initialized', { roomId: s.roomId, seat: 0, playerName: s.players[0]?.name, status: s.status }); return json({ room: publicRoom(s), player_id: 0, player_token: token }, 201);
      }
      s = await this.state();
      ...
      if (request.method === 'POST' && parts.at(-1) === 'start') { if (seat !== s.hostSeat) return json({ error: 'Only the host can start' }, 403); startGame(s); s.stateVersion++; await this.save(s); await this.broadcast(s, 'game_started'); console.log('game_started', { roomId: s.roomId, gameId: s.gameId, players: s.players.filter(Boolean).length, currentPlayer: s.currentPlayer, stateVersion: s.stateVersion }); return json({ room: publicRoom(s), state: viewFor(s, seat) }); }
      if (url.pathname.endsWith('/ws') || (parts[0] === 'ws' && parts[1] === 'rooms' && parts[2] === s.roomId)) return this.acceptWebSocket(request, s, seat);
      return json({ error: 'Not found' }, 404);
    } catch (error) { console.error('room_request_failed', { roomId, error: error instanceof Error ? error.stack ?? error.message : String(error) }); return json({ error: error instanceof Error ? error.message : 'Bad request' }, 400); }
  }
  private acceptWebSocket(request: Request, s: RoomState, seat: number) {
    ...
    console.log('socket_connected', { roomId: s.roomId, seat, sockets: this.sockets.size, status: s.status });
  ...
  private async disconnected(seat: number) { const s = await this.state(); if (s.players[seat]) { s.players[seat]!.connected = false; await this.save(s); await this.broadcast(s, 'room_updated'); console.log('socket_disconnected', { roomId: s.roomId, seat, sockets: this.sockets.size }); } }
  private async onMessage(socket: WebSocket, seat: number, raw: string) {
    try { const message = JSON.parse(raw) as { type: string; command_id?: string; expected_version?: number; action?: Action | number }; if (message.type === 'ping') return void socket.send(JSON.stringify({ type: 'pong' })); const s = await this.state();
      ...
      let actionSummary: unknown = undefined;
      if (message.type === 'make_action' && message.action !== undefined) { const action: Action = typeof message.action === 'number' ? (message.action === 0 ? { action_type: 'challenge' } : message.action === 1 ? { action_type: 'believe' } : { action_type: 'bid', quantity: Math.floor((message.action - 2) / 6) + 1, value: ((message.action - 2) % 6) + 1 }) : message.action; actionSummary = action; applyAction(s, seat, action); } else if (message.type === 'continue_round') continueRound(s, seat); else throw new Error('Unknown command');
      s.stateVersion++; const event = { type: s.status === 'finished' ? 'game_finished' : 'state_updated', room: publicRoom(s), state: viewFor(s, seat) }; s.processed[message.command_id] = event; await this.save(s); await this.broadcast(s, event.type); console.log('game_action_applied', { roomId: s.roomId, gameId: s.gameId, seat, messageType: message.type, action: actionSummary, round: s.round, currentPlayer: s.currentPlayer, status: s.status, stateVersion: s.stateVersion, historyLength: s.history.length }); if (s.status === 'finished') await this.persistHistory(s);
    } catch (error) { console.error('game_action_rejected', { roomId: (await this.state().catch(() => undefined))?.roomId, seat, error: error instanceof Error ? error.stack ?? error.message : String(error) }); socket.send(JSON.stringify({ type: 'action_rejected', error: error instanceof Error ? error.message : 'Invalid command' })); }
  }
  private async persistHistory(s: RoomState) { if (!s.gameId) return; const now = Date.now(); const statements = [this.env.DB.prepare('INSERT OR IGNORE INTO games (id, room_id, started_at, finished_at, winner_seat, turns, status, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(s.gameId, s.roomId, now, now, s.winner, s.history.length, 'finished', now + 2_592_000_000)]; for (const p of s.players) if (p) statements.push(this.env.DB.prepare('INSERT OR IGNORE INTO game_players (game_id, seat, display_name, token_hash, is_winner) VALUES (?, ?, ?, ?, ?)').bind(s.gameId, p.seat, p.name, p.tokenHash, p.seat === s.winner ? 1 : 0)); for (const h of s.history) statements.push(this.env.DB.prepare('INSERT OR IGNORE INTO game_actions (game_id, turn_number, player_seat, type, payload_json, consequences_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(s.gameId, h.turn_number, h.player_id, h.action_type, JSON.stringify(h.action_data), JSON.stringify(h.consequences), now)); await this.env.DB.batch(statements); console.log('game_history_persisted', { roomId: s.roomId, gameId: s.gameId, turns: s.history.length, winner: s.winner }); }
