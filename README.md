...
Детальная система наград для обучения агента:

### Основные награды:

- **Победа в игре**: +250.0 (когда агент остается последним игроком)
- **Потеря кости**: -6.0 за каждую потерянную кость

### Награды за действия:

- **Успешный challenge/pacao**: +2.0 (когда агент успешно поймал блеф оппонента)
- **Неудачный challenge/pacao**: -2.0 (когда агент вызвал и ошибся, что привело к потере кости)

### Промежуточные награды (в конце раунда):

- **Раунд без потери кости**: +0.5 (когда агент не потерял кость в раунде)
- **Успешный блеф**: +1.5 (когда агент сделал ставку, которая не была вызвана или была успешно защищена)
- **Неудачная ставка**: -1.5 (когда ставка агента была успешно вызвана оппонентом)

### Deferred награды (отложенные):

- **Защита ставки от challenge**: +2.5 (когда оппонент вызвал ставку агента и ошибся)
- **Защита ставки от pacao**: +5.0 (когда оппонент вызвал pacao на ставку агента и ошибся)

### Преимущество по костям:

- **Преимущество по костям**: +0.2 за каждую кость преимущества над средним количеством костей оппонентов (максимум +1.0 для 5 костей преимущества)

### Невалидные действия:

- **Невалидное действие**: -1.0 (штраф за попытку сделать недопустимое действие)
...
- **Observation space**: Поддерживает до 8 игроков (max_num_players), что позволяет обрабатывать переменное количество игроков в эпизодах
- **Action space**: 2 специальных действия (challenge, pacao) + 180 возможных ставок (30×6)
...
python -m src.perudo.training.train --device cpu

# Указать конкретный GPU
python -m src.perudo.training.train --device cuda:0
```
...
- `believe_count` - количество вызовов pacao
