...
import { DiceValue } from '../services/api';
...
export const Dice: React.FC<DiceProps> = ({
  ...
  const borderClasses = 'border-2 border-gray-400';
  ...
  const dot = (className?: string) => <div className={clsx("w-2 h-2", "rounded-full", "bg-white", className)}></div>;

  const patterns: { [key in DiceValue]: React.ReactNode } = {
    1: <span className={clsx("font-bold leading-none select-none relative bottom-0.5", "text-white", "text-3xl")}>★</span>,
    2: (
  ...
  return (
    <div className={clsx(baseClasses, borderClasses, 'bg-red-600')}>
      {patterns[value]}
    </div>
  );
};
